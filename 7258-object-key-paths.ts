...
type Dot<T extends boolean> = T extends true ? '.' : '';

type HeadPath<T extends object, U extends string | number, S extends boolean = false> = T extends any[]
    ? `${Dot<S>}${U}` | `${'' | Dot<S>}[${U}]`
    : `${Dot<S>}${U}`;

type RestPath<T> = T extends object ? '' | ObjectKeyPaths<T, true> : '';

type ObjectKeyPaths<T extends object, U extends boolean = false> = Extract<
    { [K in keyof T]: `${HeadPath<T, K & (string | number), U>}${RestPath<T[K]>}` }[keyof T],
    string
>;
