...
export type First<S extends string> = S extends `${infer F}${string}` ? F : never;

type PercentageParser<S extends string> = S extends `${Sign}${infer N}%`
    ? [First<S>, N, '%']
    : S extends `${infer N}%`
    ? ['', N, '%']
    : S extends `${Sign}${infer N}`
    ? [First<S>, N, '']
    : ['', S, ''];
