...
type Merge<T extends object, K extends PropertyKey, V> = K extends keyof T
    ? T[K] extends any[]
        ? V extends T[K][number]
            ? T
            : MergeInsertions<Omit<T, K> & Record<K, [...T[K], V]>>
        : Equal<T[K], V> extends true
        ? T
        : MergeInsertions<Omit<T, K> & Record<K, [T[K], V]>>
    : MergeInsertions<T & Record<K, V>>;

type ParseQueryString<T extends string, U extends object = {}> = T extends `${infer K}=${infer V}&${infer R}`
    ? ParseQueryString<R, Merge<U, K, V>>
    : T extends `${infer K}=${infer V}`
    ? K extends `${infer K}&${infer R}`
        ? ParseQueryString<`${R}=${V}`, Merge<U, K, true>>
        : Merge<U, K, V>
    : T extends `${infer K}&${infer R}`
    ? ParseQueryString<R, Merge<U, K, true>>
    : T extends ''
    ? U
    : Merge<U, T, true>;
