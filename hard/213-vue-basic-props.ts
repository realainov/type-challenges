...
type PropOptions = { type?: PropType };

type Prop = PropType | PropOptions;

type InferConstructorType<T> = T extends StringConstructor
    ? string
    ...
    ? boolean
    : T extends any[]
    ? { [K in keyof T]: InferConstructorType<T[K]> }[number]
    : T extends Constructor
    ? InstanceType<T>
    : any;

type InferPropType<T extends Prop> = T extends PropOptions
    ? InferConstructorType<T['type']>
    : T extends PropType
    ? InferConstructorType<T>
    : any;

type PropsThis<P extends Record<string, Prop>> = { [K in keyof P]: InferPropType<P[K]> };

declare function VueBasicProps<
    P extends Record<string, Prop> = {},
    D extends Record<string, any> = {},
    C extends Record<string, Function> = {},
    M extends Record<string, Function> = {},
    T = PropsThis<P> & D & ComputedThis<C> & M
>(options: { props?: P; data?(this: PropsThis<P>): D; computed?: C & ThisType<T>; methods?: M & ThisType<T> }): any;
