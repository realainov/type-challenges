...
type Many = number | number[];

type UnwrapMany<T> = T extends any[] ? T[number] : T;

type Intersection<T extends Many[], U = any, S extends boolean = false> = T extends [infer F, ...infer R extends any[]]
    ? Intersection<R, S extends false ? UnwrapMany<F> : U & UnwrapMany<F>, true>
    : U;
